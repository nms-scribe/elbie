...
fn test_bags() {
    ...
    assert_eq!(bag_union1.to_vec(), vec![1, 2, 3, 4, 6]);
    let bag_union2 = bag_b.union(&bag_a);
    assert_eq!(bag_union2.to_vec(), vec![1, 2, 3, 4, 6]);
    let bag_intersection1 = bag_a.intersection(&bag_b);
    assert_eq!(bag_intersection1.to_vec(), vec![2, 4]);
    let bag_intersection2 = bag_b.intersection(&bag_a);
    assert_eq!(bag_intersection2.to_vec(), vec![2, 4]);
    let bag_difference1 = bag_a.difference(&bag_b);
    assert_eq!(bag_difference1.to_vec(), vec![1, 3]);
    let bag_difference2 = bag_b.difference(&bag_a);
    assert_eq!(bag_difference2.to_vec(), vec![6]);
}
