...
pub(crate) type Axisses<'language> = Option<(&'language [(&'static str, &'static str)],
                                             Option<(&'language [(&'static str, &'static str)],
                                                     Option<(&'language [(&'static str, &'static str)], Option<&'language [(&'static str, &'static str)]>)>)>)>;
...
impl<'language> TableBuilder<'language> {
    ...
    pub fn axis(mut self, new_axis: &'language [(&'static str, &'static str)]) -> Result<Self, ElbieError> {
        self.axisses = match self.axisses {
            Some((_axis_1, Some((_axis_2, Some((_axis_3, Some(_axis_4))))))) => return Err(ElbieError::TooManyAxisses),
            Some((axis_1, Some((axis_2, Some((axis_3, None)))))) => Some((axis_1, Some((axis_2, Some((axis_3, Some(new_axis))))))),
            Some((axis_1, Some((axis_2, None)))) => Some((axis_1, Some((axis_2, Some((new_axis, None)))))),
            Some((axis_1, None)) => Some((axis_1, Some((new_axis, None)))),
            None => Some((new_axis, None))
        };
    ...
    pub fn add(self) -> Result<(), ElbieError> {
        ...
        self.language.tables_mut().push(TableEntry { id: self.id,
                                                     set: self.master_set,
