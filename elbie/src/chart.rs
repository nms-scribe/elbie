...
impl ChartStyle {
    ...
    pub(crate) fn get_phoneme_text(&self, phoneme: String) -> String {
        match self {
            Self::Plain |
            Self::Terminal |
            Self::Markdown => phoneme,
            Self::LaTeX => format!(latex!(ipa{?}),phoneme),
        }
    ...
    fn display_header_break(&self, f: &mut fmt::Formatter<'_>, columns: &[usize]) -> fmt::Result {
        match self {
            Self::Plain |
            Self::LaTeX |
            Self::Terminal => Ok(()),
    ...
    fn render_text(text: &str, is_header: bool, style: &ChartStyle, col_span: usize, row_span: usize) -> String {
        ...
        match style {
            ...
            ChartStyle::LaTeX => match (col_span, row_span,text.is_empty(),is_header) {
                ...
                (_, _, true, _) => String::new(),
                (_, _, false, true) => format!(latex!(textbf{?}),text),
                (_, _, false, false) => text.to_owned(),
            }
...
trait ChartRow {
    ...
    fn display_inside(&self, f: &mut fmt::Formatter<'_>, style: &ChartStyle, columns: &[usize], row_span: &mut usize) -> fmt::Result;



    fn display(&self, f: &mut fmt::Formatter<'_>, style: &ChartStyle, columns: &[usize], row_span: &mut usize) -> fmt::Result {
...
impl ChartRow for ChartBodyRow {
    ...
    #[allow(clippy::indexing_slicing,reason="This all works, and getting rid of that in this function is going to take a bit of work.")]
    fn display_inside(&self, f: &mut fmt::Formatter<'_>, style: &ChartStyle, columns: &[usize], row_span: &mut usize) -> fmt::Result {
...
impl ChartHeaderRow {
    ...
    const fn include_row_header(&mut self) {
...
impl Chart {
    ...
    pub(crate) const fn new(style: ChartStyle) -> Self {
        Self {
            style,
            col_headers: None,
            children: Vec::new()
    ...
    pub(crate) fn add_col_header_cell(&mut self, text: &str, col_span: usize) {
    ...
    pub(crate) fn add_cell(&mut self, text: &str) {
    ...
    /*
    pub(crate) fn add_row_header_cell(&mut self, text: &str, row_span: usize) {
        self.add_row_header_cell_at_maybe(None, text, row_span)
    }
    */

    pub(crate) fn add_row_header_cell_at(&mut self, index: usize, text: &str, row_span: usize) {
