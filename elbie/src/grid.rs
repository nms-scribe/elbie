...
impl Grid {
    ...
    fn into_html(mut self, with_span: bool) -> html_builder::Buffer {
        ...
        let row_header_offset = self.body.first().map_or(0, |first| first.headers.len());
    ...
    fn into_csv(self) -> Vec<csv::StringRecord> {
        ...
        for row in self.body {
            ...
            if let Some((header, subheader)) = row.headers {
                match header {
                    RowHeader::RowHeader { text,
                                           rowspan: _ } => record.push_field(&text),
    ...
    fn blend_column_groups(self) -> Self {
        ...
        match head {
            ...
            Some((head, None)) => {
                let mut column_groups = head.into_iter().map(|ch| (ch, Vec::new())).collect::<Vec<_>>();

                let mut new_rows = Vec::new();
                for (row_idx, row) in body.into_iter().enumerate() {
                    new_rows.push(GridRow { class: row.class,
                                            headers: row.headers,
                                            cells: Vec::new() });
                    ...
                    for (group_idx, (column_header, column_cells)) in column_groups.iter_mut().enumerate() {
                        ...
                        for idx in 0..column_header.colspan.get() {
                            if let Some(Cell { text,
                                               class: _ }) = cells_iter.next()
                            {
    ...
    fn blend_row_groups(self) -> Self {
        ...
        while let Some(mut row) = body_iter.next() {
            if let Some((RowHeader::RowHeader { text: header_text,
                                                rowspan: _ },
                         None)) = row.headers
            {
                ...
                new_body.push(GridRow { class: row.class,
        ...
        for body_row in &self.body {
            ...
            if let Some((header, subheader)) = &body_row.headers {
                match header {
                    ...
                    RowHeader::RowHeaderSpan => row.add_cell(PrettyCell::new(""))
                }
    ...
    fn pretty_table_markdown() -> PrettyTableFormat {
        PrettyFormatBuilder::new().column_separator('|').separator(LinePosition::Title, LineSeparator::new('-', '|', '|', '|')).left_border('|').right_border('|').padding(1, 1).build()
    }
