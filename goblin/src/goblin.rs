...
pub(crate) fn create_goblin_language() -> Result<Language, ElbieError> {
    ...
    language.new_table("consonant", CONSONANT, "Consonants (unvoiced ~ voiced / unaspirated ~ aspirated)")
            ...
            .axis(&[("Nasal", NASAL),
                    ("Plosive", PLOSIVE),
                    ("Fricative", FRICATIVE),
                    ("Reversed Affricate", REV_AFFRICATE),
                    ("Approximant", NONLATERALAPPROXIMANT),
                    ("Lateral", LATERAL),
                    ("Tap", TAP_OR_FLAP)])?
...
pub(crate) mod to_hobgoblin {
    ...
    pub(crate) fn create_goblin_to_hobgoblin(family: &mut Family) -> Result<Transformation, ElbieError> {
        ...
        transformation.add_rule("aspirated to affricate", |rule| {
                          ...
                          _ = rule.opt_repl(V_ASPIR, &[PFA])? || rule.opt_repl(Z_ASPIR, &[TSA])? || rule.opt_repl(GAMMA_ASPIR, &[KXA])?;

                          Ok(true)
                      });

        transformation.add_rule("palatalize and break affricates", |rule| {
                          _ = rule.opt_repl(PFA, &[P])? || rule.opt_repl(TSA, &[T, ESH])? || rule.opt_repl(KXA, &[K, ESH])? || rule.fail()?;
        ...
        transformation.add_rule("palatalize and break reverse affricates", |rule| {
                          ...
                          _ = rule.opt_repl(TIE_X_K, &[ESH, K])? || rule.opt_repl(TIE_GAMMA_G, &[EZH, G])? || rule.fail()?;
        ...
        transformation.add_rule("syllabification (1)", |rule| {
                          ...
                          _ = rule.opt_repl(OPEN, &[])? || rule.opt_repl(OPEN_MID, &[])? || rule.fail()?;
        ...
        transformation.add_rule("syllabification (4)", |rule| {
                          ...
                          _ = rule.opt_repl(J_SYL, &[I])? || rule.opt_repl(CURSIVE_V_SYL, &[U])? || rule.opt_repl(TURNED_M_RIGHT_LEG_SYL, &[A])? || rule.fail()?;
